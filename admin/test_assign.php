<?php
/**
 * Admin — Assign Test to Candidates
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'assign') {
    ...
    if (empty($candidateIds)) {
        flash('test_assign', 'Please select at least one candidate.', 'error');
    } else {
        $assignedCount = 0;
        foreach ($candidateIds as $cId) {
            if (createTestInvitation($testId, (int)$cId, $adminId)) {
                $assignedCount++;
            }
        }
        
        if ($assignedCount > 0) {
            flash('test_assign', "Successfully sent test invitations to $assignedCount candidate(s).", 'success');
        } else {
            flash('test_assign', 'No new invitations sent. Candidates may already be assigned.', 'warning');
        }
    }
    redirect(BASE_URL . "/admin/test_assign.php?id=$testId");
}

// Get all active candidates
$candidatesData = getAllCandidates('', 'active', 1, 1000);
$candidates = $candidatesData['rows'];

// Get current invitations for this test
$db = getDB();
$stmt = $db->prepare("SELECT ti.*, u.full_name, u.email 
                      FROM test_invitations ti 
                      JOIN users u ON ti.candidate_id = u.id 
                      WHERE ti.test_id = ? 
...
renderHeader('Assign Test', 'dashboard-page');
...
<div class="page-header d-flex align-items-start justify-content-between">
  <div>
    <div class="d-flex align-items-center gap-3 mb-2">
      ...
      <h1 class="page-header__title mb-0">Assign Test</h1>
    </div>
    <p class="page-header__subtitle">Assign <strong><?= e($test['title']) ?></strong> to candidates</p>
  </div>
</div>

<?php renderAlert(getFlash('test_assign')); ?>
    ...
    <div class="content-card" style="padding: 1.5rem;">
      ...
      <form method="POST" action="">
        ...
        <div class="mb-3">
          ...
          <div class="form-text">Select one or more candidates to send a test invitation. This will generate a notification for them.</div>
        </div>
